...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Se \(X\) è alla fine di una produzione, si aggiunge il FOLLOW della variabile precedente.
    \end{itemize}
\end{itemize}
...
Dove:
\begin{itemize}
    \item $V$ è l'insieme dei simboli non terminali della grammatica.
...
id \quad \= F \quad \= F \quad \= T * F \quad \= T \\
id \quad \= F \quad \= id \quad \= T * F \\
id \quad \= F \quad \= id \\
id
\end{tabbing}
...
id \quad \= F \quad \= T \quad \= T + id \quad \= T \quad \= F \quad \= E + T \\
id \quad \= F \quad \= F \quad \= F \quad \= id \quad \= T \quad \= F \\
id \quad \= id \quad \= id \quad \= F \quad \= id \\
id
\end{tabbing}
...
Problema: quando in cima allo stack c'è la parte destra di una regola, come si fa a sapere se è l’handle e si deve fare la riduzione oppure è necessario spostare ancora simboli?  
In cima allo stack potrebbero esserci parti destre di regole diverse: quale si sceglie?

Il parsing usato comunemente è LR(k):
\begin{itemize}
